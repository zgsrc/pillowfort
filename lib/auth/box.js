...
function getLoginHandler(appId, callbackUrl, permissions) {
    return function(req, res, next) {
        req.session.box = { state: uuid().toString() };
        req.session.save(function() {
            res.redirect(redirectUrl(appId, callbackUrl, permissions, req.session.box.state));
        });
    };
}
...
function getCallbackHandler(appId, appSecret, callbackUrl) {
    return function(req, res, next) {
        if (req.query.error) {
            ...
            req.session.box = Object.merge(req.session.box, {
                scopes: req.query.granted_scopes.split(","),
                code: req.query.code
            });

            if (!req.query.state || req.query.state == req.session.box.state) {
                getAccessToken(appId, appSecret, callbackUrl, req.query.code, function(err, data) {
                    if (err) req.error = err;
                    else req.session.box = Object.merge(req.session.box, data);

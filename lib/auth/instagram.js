...
function getLoginHandler(appId, callbackUrl, permissions) {
    return function(req, res, next) {
        req.session.instagram = { state: uuid().toString() };
        req.session.save(function() {
            res.redirect(redirectUrl(appId, callbackUrl, permissions, req.session.instagram.state));
        });
    };
}
...
function getCallbackHandler(appId, appSecret, callbackUrl) {
    return function(req, res, next) {
        if (req.query.error) {
            ...
            req.session.instagram = Object.merge(req.session.instagram, {
                scopes: req.query.granted_scopes.split(","),
                code: req.query.code
            });

            req.session.save(function() {
                if (!req.query.state || req.query.state == req.session.instagram.state) {
                    accessToken(appId, appSecret, callbackUrl, req.query.code, function(err, data) {
                        if (err) req.error = err;
                        else req.session.instagram = Object.merge(req.session.instagram, data);
                        next();
                    });
                }
                else {
